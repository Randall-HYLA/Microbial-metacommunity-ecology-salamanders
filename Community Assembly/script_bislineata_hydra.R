...
otu.table <- read.csv("bislineata_otu_table_rare.csv", header = T, check.names = F, row.names = 1)
otu.matrix <- as.matrix(otu.table)

samp.meta <- read.csv("bislineata_metadata_rare.csv", head = T, sep = ",")

#Load phylogenetic tree
library(picante)
tree <- read.tree(file="bislineata_final_root.nwk")
