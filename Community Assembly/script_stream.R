...
# Part 2: 
com <- read.csv("stream_otu_table_rare.csv", header = T, check.names = F, row.names = 1)
...
bNTI<-read.table("bNTI.group_stream.csv",header=T,sep=",", check.names = F, row.names=1)
