...
otu.table <- read.csv("cinereus_otu_table_rarefy.csv", header = T, check.names = F, row.names = 1)
otu.matrix <- as.matrix(otu.table)

samp.meta <- read.csv("cinereus_metadata.csv", head = T, sep = ",")

#Load phylogenetic tree
library(picante)
tree <- read.tree(file="cinereus_final_root.nwk")
...
write.csv(as.matrix(bMNTD),file="bMNTD_cinereus.csv")# output observed betaMNTD
...
write.csv(bNTI.all,file="bNTIall_cinereus.csv")# output
