...
otu.table <- read.csv("pond_otu_table_rare.csv", header = T, check.names = F, row.names = 1)
otu.matrix <- as.matrix(otu.table)

samp.meta <- read.csv("pond_metadata_rare.csv", head = T, sep = ",")

#Load phylogenetic tree
library(picante)
tree <- read.tree(file="pond_final_root.nwk")
...
write.csv(bNTI.group$betaNTI,file="bNTI.group_pond.csv")
write.csv(bNTI.group$betaMNTD,file="bMNTD.group_pond.csv")
